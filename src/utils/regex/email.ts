...
export function isValidEmail(email: string, excludeSubstrings?: string | RegExp | string[]): boolean {
    if (!email) return false;
    email = email.trim();
    return EMAIL_REGEX.test(email) 
        && (excludeSubstrings ? !stringContainsAnyOf(email, excludeSubstrings) : true);
}
